...
function App() {
  ...
  const editValue=(userId)=>{
    ...
    // console.log(newData)
    let updatedUserList = userData.map(
      user=>{
        if(user.id===userId){
          return {...user, ...newData};
        }
  ...
  return (
    <div className="p-4 h-screen w-screen">
      <div className="flex justify-between items-center h-4">
        ...
        <Icon icon="icon-park-twotone:delete" color="red" width="26" height="26" />
        </div>
      </div>
      <div className="col flex justify-evenly items-center my-4 border border-solid border-gray-200">
          
          <input type="checkbox" className='w-1/12'/>
      ...
      </div>
      {
        currentItems.map((user)=>{
          return (
            <div key={user.id} className={'flex justify-evenly items-center p-2 border border-solid border-gray-200'}>
                <input type="checkbox" className='w-1/12'/>
                {editableRow === user.id ? (
                  <Input 
                ...
                <div className='flex w-2/12 justify-start text-centre'>
                  <div 
                    ...
                    >
                      <Icon icon="ic:twotone-delete" color="red" width="26" height="26" />
                  </div>
                   
                    {
                      ...
                      <>
                        ...
                        className=" h-max w-max ">
                          <Icon icon="mingcute:save-fill" color="lightgreen" width="26" height="26" />
                          </button>
                        <button 
                        onClick={handleCancelClick}
                        className="mr-1 h-max w-max"
                        >
                          <Icon icon="basil:cancel-solid" color="red" width="28" height="28" />
                        </button>
                      </> :
                      <>
                        <button 
                        ...
                        >
                          <Icon icon="uil:edit" color="white" width="26" height="26" />
                        </button>
                      </>
                    }
      ...
      <div className="footer flex justify-between w-full fixed bottom-3 right-2 left-2">
        <div className="">
          0 of {userData.length} row(s) selected.
        </div>

        <div className="flex justify-center items-center ">
            <h5 className="font-semibold mr-2">Page {currentPage+1} of {totalPages} </h5>
            <div 
      ...
      onClick={()=>setCurrentPage(totalPages-1)}>
              <Icon icon="wpf:first" color="skyblue" width="26" height="26" rotate={2} /> 
            </div>
        </div>
      </div>
